...
foreach ($cart as $key => $value) {
    ...
    $idbarang = $_SESSION['cart'][$key]['id_barang'];
    //cek diskon barang
    $disbarang = mysqli_query($koneksi, "SELECT * FROM disbarang WHERE barang_id='$idbarang'");
    $disb = mysqli_fetch_assoc($disbarang);

    //cek jika di keranjang sudah ada barang yang masuk
    $key = array_search($idbarang, array_column($_SESSION['cart'], 'id_barang'));
    // return var_dump($key);
    if ($key !== false) {
